...
def run_instance(fuzzer_name, output_folder, num_iterations, use_multiple_graphs,
                 feedback_check_type, scheduler_type, instance_index):
...
def main():
    ...
    parser.add_argument("--feedback_check_type", type=str,
                        choices=["regular", "coverage", "combination", "branch", "branchhit", "none"],
                        default="regular", help="The type of feedback check to use: "
                                                ...
                                                "'branch' for coverage.py branch feedback, "
                                                "'branchhit' for branch hit-count feedback, "
    ...
    for fuzzer_name, output_folder, num_instances in fuzzer_configs:
        ...
        for i in range(1, num_instances + 1):
            p = multiprocessing.Process(target=run_instance, args=(
                ...
                args.feedback_check_type, args.scheduler, i))
            processes.append(p)
            p.start()
