...
class STPLFuzzer(BaseFuzzer):
    def __init__(self, *args, **kwargs):
        ...
        self.set_interesting_check(self.shortest_path_interesting_check)
    ...
    @staticmethod
    def shortest_path_interesting_check(result):
        if result == float("inf"):
            return ("SPF_NO_PATH",)
        if isinstance(result, int):
            return result
        raise ValueError(f"Unknown shortest-path result type: {type(result)}")
    ...
    def process_test_results(self, mutated_graph, tester, first_occurrence_times,
                             ...
                             ops=None):
        discrepancies = tester.test_single_graph(
            mutated_graph, timestamp, num_pairs=10, parent_id=parent_id,
            ops=ops, bug_dir=self.bug_dir)
