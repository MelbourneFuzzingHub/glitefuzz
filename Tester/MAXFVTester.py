...
from Utils.FileUtils import save_bug, save_discrepancies, save_discrepancy
...
class MAXFVTester:
    ...
    def test_single_graph(self, G, timestamp=None, parent_id=None, ops=None,
                          bug_dir=None):
        discrepancies = self.run_maxfv_tests_multiple_times(
            G, timestamp=timestamp, parent_id=parent_id, ops=ops,
            bug_dir=bug_dir)
        if discrepancies:
            return discrepancies
        return None

    def run_maxfv_tests_multiple_times(self, G, num_runs=5, timestamp=None,
                                       parent_id=None, ops=None, bug_dir=None):
        ...
        for _ in range(num_runs):
            ...
            if discrepancy is not None:
                ...
                if bug_dir:
                    save_bug(graph, timestamp, parent_id, ops, bug_dir,
                             discrepancy_message)
                save_discrepancy(
